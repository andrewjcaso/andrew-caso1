...
(() => {
  // READ is resolved against THIS script's location so a page in a category
  // subfolder still sees the one root sidecar (fetch handles '../' fine).
  // WRITE must stay a bare basename — the host allowlists basenames only and
  // rejects any path with a separator. Note the host then resolves that name
  // against the PAGE's folder, so writes from a subfolder page currently fail
  // host-side ('write failed'); author images via a real src= for those.
  const STATE_NAME = '.image-slots.state.json';
  const STATE_FILE = (() => {
    const s = (document.currentScript && document.currentScript.getAttribute('src')) || '';
    const i = s.lastIndexOf('/');
    return (i >= 0 ? s.slice(0, i + 1) : '') + STATE_NAME;
  })();
  ...
  function save() {
    ...
    if (!w) return;
    saving = true;
    Promise.resolve(w(STATE_NAME, JSON.stringify(slots)))
      .catch(() => {})
  ...
  const stylesheet =
    ...
    '  white-space:nowrap}' +
    ':host([data-reframe]) .ctl:not(:empty){opacity:1;pointer-events:auto}' +
  ...
  class ImageSlot extends HTMLElement {
    ...
    // Public: set the user-image URL programmatically. Used by the
    // video-slot poster bridge to fill grid hovers from a captured frame.
    setUserUrl(url) {
      if (!url) return;
      const val = { u: url, s: 1, x: 0, y: 0 };
      if (this.id) setSlot(this.id, val);
      else { this._local = val; this._render && this._render(); }
    }
    ...
    _render() {
      ...
      if (url) {
        ...
        // Belt-and-braces: the host attribute can be stripped by a host
        // framework re-render, so hide the placeholder ring inline too.
